...
pub struct PlanetParams {
    pub seed:                   u32,
    // Elevation
    pub radius:                 f64, // Earth radii; scales feature sizes
    pub warp_strength:          f64,
    // Climate
    pub temp_baseline:          f64, // [0,1] equatorial warmth; from insolation + greenhouse
    pub temp_gradient:          f64, // how steeply temperature falls pole-ward; 1.0 = maximum
    pub lapse_factor:           f64, // elevation cooling rate
    pub precip_moisture:        f64, // [0,1] ocean evaporation ceiling; from WaterVapor fraction
    pub land_decay:             f64, // moisture loss per land cell
    pub slope_threshold:        f64, // min elevation gain to trigger rain shadow
    pub slope_loss:             f64, // rain shadow intensity
    pub base_arid:              f64, // minimum interior precipitation
    pub et_factor:              f64, // evapotranspiration scaling
    pub glacier_temp_threshold: f64, // temp below which land glaciates
    pub sea_ice_temp_threshold: f64, // temp below which ocean freezes
    pub sea_ice_evap_factor:    f64, // ocean evaporation reduction under sea ice
    // Atmosphere
    pub gravity:                f64, // surface gravity in Earth g; used for pressure-at-elevation
    pub base_press:             f64, // sea-level atmospheric pressure in kPa
    pub atmo:                   HashMap<AtmosphereTag, f32>, // atmospheric composition fractions
    // Hydrology
    pub sea_level:              f64, // from liquid_coverage
    pub max_lake_fill:          f64,
    pub aquifer_probability:    f64, // from Carbonate/Icy geo fraction
    ...
    pub lon_weight:             f64,   // how much east-west spread from seed costs; 0 = no limit
    // Salt flats
    pub salt_flat_probability:  f64,   // from Crystalline geo fraction; raises aridity threshold for salt flat formation
    // Orbital mechanics
    pub axial_tilt:             f64,   // degrees; drives ITCZ migration amplitude
    pub orbital_period:         f64,   // standard days per planetary year; used for season_phase
    pub rotation_period:        f64,   // standard days per planetary day (informational for now)
}

impl PlanetParams {
    /// Earth-analog defaults — exactly reproduces the previous hardcoded behaviour.
    pub fn earth_like(seed: u32) -> Self {
        Self {
            seed,
            radius:                 1.0,
            warp_strength:          0.2,
    ...
    pub fn from_planet(planet: &Planet, star: &Star) -> Self {
...
pub struct SeasonalHydro {
    ...
    pub flow_phasor: Vec<PrecipPhasor>,
}

/// Evaluate a PrecipPhasor at a given season angle.
pub fn sample_precip_phasor(p: &PrecipPhasor, season_phase: f64) -> f32 {
...
pub fn generate_seasonal_precip(
    elevation: &HeatMap,
    is_ocean: &[bool],
    temperature: &HeatMap,
    is_sea_ice: &[bool],
...
pub fn generate_seasonal_hydro(
...
impl HeatMap {
    pub fn generate_elevation(width: usize, height: usize, seed: u32, warp_strength: f64) -> Self {
    ...
    pub fn generate_temperature(elevation: &HeatMap, params: &PlanetParams, season_phase: f64) -> HeatMap {
    ...
    pub fn generate_precipitation(elevation: &HeatMap, is_ocean: &[bool], temperature: &HeatMap, is_sea_ice: &[bool], params: &PlanetParams, season_phase: f64) -> HeatMap {
        let width = elevation.width;
        let height = elevation.height;
        let n = width * height;
...
pub fn elevation_color(t: f64) -> [u8; 3] {
    sample_gradient(
        t,
        &[
            ([255, 0, 0], 0.00),
...
pub fn terrain_color(land_t: f64) -> [u8; 3] {
    sample_gradient(
        land_t,
        &[
...
pub fn temperature_color(t: f64) -> [u8; 3] {
    sample_gradient(
        t,
        &[
...
pub fn precipitation_color(t: f64) -> [u8; 3] {
    sample_gradient(
        t,
        &[
...
pub fn sea_ice_color(t: f64, threshold: f64) -> [u8; 3] {
...
pub fn glacier_color(t: f64, threshold: f64) -> [u8; 3] {
    let normalized = (t / threshold).clamp(0.0, 1.0);
    sample_gradient(
        normalized,
        &[
...
pub fn habitability_color(t: f64) -> [u8; 3] {
    if t <= 0.0 {
        return [45, 45, 45];
    }
    if t >= 1.0 {
...
pub fn aridity_color(t: f64) -> [u8; 3] {
    sample_gradient(
        t,
        &[
...
pub fn bayer_dither(t: f64, rx: usize, ry: usize, n_levels: usize) -> f64 {
...
pub fn seed_from_uuid(bytes: [u8; 16]) -> u32 {
...
pub fn political_color(region_id: u32) -> [u8; 3] {
...
pub fn score_region_for_species(species: &Species, region: &Region, params: &PlanetParams) -> f64 {
...
pub fn compute_salt_flat_dist(is_salt_flat: &[bool], width: usize, height: usize) -> Vec<u32> {
...
pub fn render_terrain_layer(
    ...
    ImageBuffer::from_fn(render_width as u32, render_height as u32, |rx, ry| {
        ...
        let rgb = if is_ocean[data_idx] {
            [20u8, 35, 55]
        } else if is_salt_flat[data_idx] {
            let coverage = salt_flat_dist[data_idx] as f64 / SALT_FLAT_DITHER_DIST as f64;
            if BAYER_4X4[ry as usize % 4][rx as usize % 4] < coverage {
                let d = bayer_dither(aridity.sample(nx, ny), rx as usize, ry as usize, N_DITHER_LEVELS);
                salt_flat_color(d)
            } else {
                let land_t = ((elevation.sample(nx, ny) - params.sea_level) / (1.0 - params.sea_level)).clamp(0.0, 1.0);
                terrain_color(bayer_dither(land_t, rx as usize, ry as usize, N_DITHER_LEVELS))
            }
        } else {
            let land_t = ((elevation.sample(nx, ny) - params.sea_level) / (1.0 - params.sea_level)).clamp(0.0, 1.0);
            terrain_color(bayer_dither(land_t, rx as usize, ry as usize, N_DITHER_LEVELS))
        ...
        let nx_r = (rx as usize + 1) as f64 / render_width as f64;
        let ny_d = (ry as usize + 1) as f64 / render_height as f64;
        let darken = if !is_ocean[data_idx]
            && is_contour(elevation.sample(nx, ny), elevation.sample(nx_r, ny), elevation.sample(nx, ny_d), N_CONTOURS)
        {
            CONTOUR_DARKEN
        } else {
            1.0
        ...
        Rgba([
            (rgb[0] as f64 * darken) as u8,
            (rgb[1] as f64 * darken) as u8,
            (rgb[2] as f64 * darken) as u8,
            255,
        ])
    })
}

pub fn render_hydrology_layer(
...
pub fn render_ice_layer(
    ...
    ImageBuffer::from_fn(render_width as u32, render_height as u32, |rx, ry| {
        ...
        if is_ocean[data_idx] && is_sea_ice[data_idx] {
            let sea_ice_neighbor = |ndx: i64, ndy: i64| -> bool {
                let nnx = ndx.rem_euclid(width as i64) as usize;
                let nny = ndy.clamp(0, height as i64 - 1) as usize;
                is_sea_ice[nny * width + nnx]
            ...
            if BAYER_4X4[ry as usize % 4][rx as usize % 4] < coverage {
                let t = temperature.sample(nx, ny);
                let d = bayer_dither(t / params.sea_ice_temp_threshold, rx as usize, ry as usize, N_DITHER_LEVELS);
                let [r, g, b] = sea_ice_color(d, params.sea_ice_temp_threshold);
                Rgba([r, g, b, 255])
            } else {
                Rgba([0, 0, 0, 0])
            }
            ...
            let non_glacier_land = |ndx: i64, ndy: i64| -> bool {
                ...
                let nny = ndy.clamp(0, height as i64 - 1) as usize;
                let nidx = nny * width + nnx;
                !is_glacier[nidx] && !is_ocean[nidx] && annual_hydro.data[nidx] <= 0.0
            };
            const GLACIER_EDGE: f64 = 0.05;
            let mut coverage = 1.0f64;
            if off_x == 0 && non_glacier_land(dx as i64 - 1, dy as i64) { coverage = GLACIER_EDGE; }
            ...
            if BAYER_4X4[ry as usize % 4][rx as usize % 4] < coverage {
                let t = temperature.sample(nx, ny);
                let d = bayer_dither(t / params.glacier_temp_threshold, rx as usize, ry as usize, N_DITHER_LEVELS);
                let [r, g, b] = glacier_color(d, params.glacier_temp_threshold);
                Rgba([r, g, b, 255])
            } else {
                Rgba([0, 0, 0, 0])
            }
